...
const MapaBrasil = ({ onEstadoClick, estadoSelecionado, estatisticasEstados, filters }) => {
  ...
  // Criar escala de cores baseada nos totais
  ...
  const getCorEstado = (nomeEstado) => {
    // Mapear nome do estado para ID (simplificado)
    const estadoMap = {
      'Acre': 12, 'Alagoas': 17, 'Amapá': 16, 'Amazonas': 13, 'Bahia': 29,
      'Ceará': 23, 'Distrito Federal': 53, 'Espírito Santo': 32, 'Goiás': 52,
      'Maranhão': 21, 'Mato Grosso': 51, 'Mato Grosso do Sul': 50, 'Minas Gerais': 31,
      'Pará': 15, 'Paraíba': 25, 'Paraná': 41, 'Pernambuco': 26, 'Piauí': 22,
      'Rio de Janeiro': 33, 'Rio Grande do Norte': 24, 'Rio Grande do Sul': 43,
      'Rondônia': 11, 'Roraima': 14, 'Santa Catarina': 42, 'São Paulo': 35,
      'Sergipe': 28, 'Tocantins': 27
    }
    ...
    if (estadoSelecionado === estadoId) return corOriginal
  ...
  const calcTooltipPosition = (x, y) => {
    ...
    if (newX + tooltipWidth + padding > vw) {
  ...
  const handleMouseEnter = (evt, nomeEstado) => {
    ...
    const estadoId = estadoMap[nomeEstado]
    if (estadoId && estatisticasEstados[estadoId]) {
      const stats = estatisticasEstados[estadoId]
      const total = stats.reduce((sum, stat) => {
        if (stat.ano !== filters.ano) return sum
        switch (filters.tipoRegistro) {
